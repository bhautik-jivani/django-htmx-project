...
from myapp.models import Publisher  # Replace with your actual model

# Create your views here.
class PublisherAPIView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if search_query:
            publisher_queryset = publisher_queryset.filter(
                Q(id__icontains=search_query) | 
                Q(name__icontains=search_query)
            )

        total_records = publisher_queryset.count()  # Total records before filtering

        # Apply sorting
        publisher_queryset = publisher_queryset.order_by(order_column)

        # Paginate results based on DataTables "start" and "length"
        paginator = Paginator(publisher_queryset, length)
